...
* Что сделал: до начала работы зашёл в `requirements.txt`, прочитал несколько статей на `habr` о `fastapi` и сопутствующих технологиях; просмотрел структуру проекта.

#### Исправление бага №1.

* Описание проблемы: для более старых версий `python` указана некорректная версия `fastapi`;
* Решение: заменить на последнюю актуальную версию для `python` < 3.8;
...
* Решение: не обращаться напрямую к `item.city.name` и заменить его на `None`, если `item.city` не задан;
...
* Описание проблемы: эндпоинт `POST /api/v1/vacancies/` для создания вакансий при совпадающих внешних `id` возвращает код 200, хотя запрос не обрабатывается корректно;
...
* Решение: заменить на контекстную обёртку или закрыть соединение после выполнения;
...
* Решение: заменить словарь на множество по аналогии с кодом выше, чтобы `id` были уникальные;
